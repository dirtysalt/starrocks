...
public:
    ...
    // Append `src` into `dst` with explicit pre-alignment check for shredded
    // columns. Returns InvalidArgument when `dst` is non-empty row mode and
    // `src` is shredded mode. Returns NotSupported when shredded schema/type
    // conflicts cannot be reconciled by current policy.
