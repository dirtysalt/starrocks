...
Status VariantColumnReader::read_range(const Range<uint64_t>& range, const Filter* filter, ColumnPtr& dst) {
    ...
    // Phase-1 canonical form for parquet variant read:
    // read base payload directly into shredded(metadata/remain), instead of rebuilding row-wise objects.
    DCHECK_EQ(variant_column->size(), 0);
    variant_column->set_shredded_columns({}, {}, {}, BinaryColumn::create(), BinaryColumn::create());
    DCHECK(variant_column->has_metadata_column());
    DCHECK(variant_column->has_remain_value());
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        if (!is_null) {
            ...
            variant_column->append_shredded(metadata_slice, value_slice);
        }
        if (is_null) {
