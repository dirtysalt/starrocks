...
static Status promote_typed_only_for_merge(VariantColumn* column) {
    if (column == nullptr) {
        return Status::InvalidArgument("column should not be null");
    }
    if (!column->is_typed_only_variant()) {
        return Status::OK();
    }

    auto metadata = BinaryColumn::create();
    auto remain = BinaryColumn::create();
    metadata->reserve(column->size());
    remain->reserve(column->size());

    for (size_t row = 0; row < column->size(); ++row) {
        VariantRowValue row_buffer;
        const VariantRowValue* variant = column->get_row_value(row, &row_buffer);
        if (variant == nullptr) {
            return Status::InvalidArgument("failed to materialize typed-only row during merge normalization");
        }
        std::string_view metadata_raw = variant->get_metadata().raw();
        std::string_view value_raw = variant->get_value().raw();
    ...
    column->set_shredded_columns(std::move(paths), std::move(type_descs), std::move(typed), std::move(metadata),
                                 std::move(remain));
    return Status::OK();
}

static Status arbitrate_shredded_type_conflicts(VariantColumn* dst, VariantColumn* src) {
    if (dst == nullptr || src == nullptr) {
        return Status::InvalidArgument("dst/src should not be null");
    }
    if (!dst->is_shredded_variant() || !src->is_shredded_variant()) {
        return Status::OK();
    }
...
static StatusOr<MutableColumnPtr> clone_shredded_variant_for_merge(const VariantColumn& src) {
    auto cloned = VariantColumn::create();
    MutableColumns typed;
    typed.reserve(src.typed_columns().size());
    for (const auto& col : src.typed_columns()) {
        typed.emplace_back(col->clone());
    }
    MutableColumnPtr metadata = nullptr;
    if (src.metadata_column() != nullptr) {
        metadata = src.metadata_column()->clone();
    }
    MutableColumnPtr remain = nullptr;
    if (src.remain_value_column() != nullptr) {
        remain = src.remain_value_column()->clone();
    }
    cloned->set_shredded_columns(src.shredded_paths(), src.shredded_types(), std::move(typed), std::move(metadata),
                                 std::move(remain));
    auto* cloned_variant = down_cast<VariantColumn*>(cloned.get());
    RETURN_IF_ERROR(promote_typed_only_for_merge(cloned_variant));
    return cloned;
}
...
Status VariantMerger::merge_into(VariantColumn* dst, const VariantColumn& src) {
    ...
    RETURN_IF_ERROR(promote_typed_only_for_merge(dst));

    const VariantColumn* src_for_merge = &src;
    MutableColumnPtr normalized_src_holder = nullptr;
    if (src.is_typed_only_variant()) {
        ASSIGN_OR_RETURN(normalized_src_holder, clone_shredded_variant_for_merge(src));
        src_for_merge = down_cast<VariantColumn*>(normalized_src_holder.get());
    }

    if (!dst->is_shredded_variant() && dst->size() > 0 && src_for_merge->is_shredded_variant()) {
        return Status::InvalidArgument("cannot merge shredded src into non-empty row dst");
    }

    if (dst->is_shredded_variant() && src_for_merge->is_shredded_variant() && !dst->is_equal_schema(src_for_merge)) {
        ASSIGN_OR_RETURN(bool has_conflict, has_overlapped_type_conflict(*dst, *src_for_merge));
        if (!has_conflict) {
            if (!dst->prepare_shredded_schema_for_append(*src_for_merge)) {
                return Status::NotSupported("failed to prepare shredded schema before merge");
            }
            dst->append(*src_for_merge, 0, src_for_merge->size());
            return Status::OK();
        }

        ASSIGN_OR_RETURN(auto src_mutable, clone_shredded_variant_for_merge(*src_for_merge));
        auto* src_variant = down_cast<VariantColumn*>(src_mutable.get());
        RETURN_IF_ERROR(arbitrate_shredded_type_conflicts(dst, src_variant));
        if (!dst->prepare_shredded_schema_for_append(*src_variant)) {
            return Status::NotSupported("failed to prepare shredded schema before merge after arbitration");
        }
        dst->append(*src_variant, 0, src_variant->size());
        return Status::OK();
    }

    if (src_for_merge->is_shredded_variant() && !dst->prepare_shredded_schema_for_append(*src_for_merge)) {
        return Status::NotSupported("failed to prepare shredded schema before merge");
    }
...
StatusOr<MutableColumnPtr> VariantMerger::merge(const Columns& inputs) {
    ...
    const VariantColumn* first_shredded = nullptr;
    for (const auto& input : inputs) {
        ASSIGN_OR_RETURN(const auto* variant, get_variant_data_column(input.get()));
        if (variant->is_shredded_variant()) {
            first_shredded = variant;
            break;
        }
    }
    if (first_shredded != nullptr) {
        RETURN_IF_ERROR(merged_variant->init_shredded_schema_from(*first_shredded));
    }
