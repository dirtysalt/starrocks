...
Status VariantColumnReader::read_range(const Range<uint64_t>& range, const Filter* filter, ColumnPtr& dst) {
    ...
    // Phase-1 compatibility lock: always use row-wise ObjectColumn<VariantRowValue>.
    variant_column->clear_shredded_columns();
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        if (!is_null) {
            ...
            if (auto variant = VariantRowValue::create(metadata_slice, value_slice); !variant.ok()) {
                VLOG_FILE << "Failed to create variant value at row " << i << ": " << variant.status();
                is_null = true;
            } else {
                VariantRowValue row = std::move(variant.value());
