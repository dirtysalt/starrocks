...
PARALLEL_TEST(VariantMergerTest, merge_shredded_schema_union) {
    ...
    ASSERT_TRUE(merged->is_shredded_variant());
...
PARALLEL_TEST(VariantMergerTest, merge_shredded_type_conflict_hoist_variant) {
    ...
    const VariantRowValue* row0 = typed_a->get(0).get_variant();
    const VariantRowValue* row1 = typed_a->get(1).get_variant();
    ...
    ASSERT_EQ("10", row0_json.value());
    ASSERT_EQ("\"x\"", row1_json.value());
}
...
PARALLEL_TEST(VariantMergerTest, merge_into_row_dst_with_existing_rows_and_shredded_src_fail) {
    ...
    auto st = VariantMerger::merge_into(dst.get(), *src);
    ASSERT_FALSE(st.ok());
...
PARALLEL_TEST(VariantMergerTest, merge_root_typed_only_with_base_shredded_ok) {
    ...
    root_typed_only->set_shredded_columns({""}, {TypeDescriptor(TYPE_BIGINT)}, std::move(root_typed_only_typed),
                                          nullptr, nullptr);
...
PARALLEL_TEST(VariantMergerTest, merge_root_typed_only_with_root_typed_only_ok) {
    ...
    src0->set_shredded_columns({""}, {TypeDescriptor(TYPE_BIGINT)}, std::move(src0_typed), nullptr, nullptr);

    auto src1 = VariantColumn::create();
    MutableColumns src1_typed;
    src1_typed.emplace_back(build_nullable_root_int64_typed_only_column({8}, {0}));
    src1->set_shredded_columns({""}, {TypeDescriptor(TYPE_BIGINT)}, std::move(src1_typed), nullptr, nullptr);
    ...
    ASSERT_TRUE(merged->is_shredded_variant());
    ASSERT_EQ(2, merged->size());
    ASSERT_TRUE(merged->shredded_paths().empty());
    ASSERT_TRUE(merged->typed_columns().empty());
