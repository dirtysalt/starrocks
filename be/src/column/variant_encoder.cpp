...
Status encode_column_with_viewer(const ColumnPtr& column, ColumnBuilder<TYPE_VARIANT>* builder,
    ...
    const RunTimeCppType<LT>* data = typed_column->get_data().data();
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        bool is_null = false;
        StatusOr<VariantRowValue> encoded = encode_value(data, data_row, encoder, &is_null);
        if (!encoded.ok()) {
...
Status VariantEncoder::encode_column(const ColumnPtr& column, const TypeDescriptor& type,
    ...
    case TYPE_BOOLEAN: {
        return encode_column_with_viewer<TYPE_BOOLEAN>(
                ...
                [&](const auto* data, size_t data_row, VariantRowValueEncoder& encoder,
                    bool* value_is_null) -> StatusOr<VariantRowValue> {
                    bool value = data[data_row];
                    uint8_t header =
    ...
    case TYPE_CHAR: {
        return encode_column_with_viewer<TYPE_CHAR>(
                ...
                [&](const auto* data, size_t data_row, VariantRowValueEncoder& encoder,
                    bool* value_is_null) -> StatusOr<VariantRowValue> {
                    return encoder.make_variant(encode_string_value(data[data_row]));
                });
    }
    case TYPE_VARCHAR: {
